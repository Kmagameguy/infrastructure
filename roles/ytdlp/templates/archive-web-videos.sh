...
ROOT_PATH="{{ video_storage_path }}"

run_download() {
  local CHANNEL_URL="${1}"
  "{{ ytdlp_binary }}" "${CHANNEL_URL}" \
  ...
  --verbose --force-ipv4 --sleep-requests 1 --sleep-interval 5 --max-sleep-interval 30 --ignore-errors --no-continue --no-overwrites --download-archive .download_history --add-metadata --match-filter "!is_live & !live" \
  --datebefore "$(date --date='30 days ago' +%Y%m%d)" \
  --output "%(title)s [%(id)s].%(ext)s" --merge-output-format "mkv" --extractor-args youtube:player_client=android --throttled-rate 100K
}
...
main() {
  for channel in "${!CHANNELS[@]}"; do
    local cur_path="${ROOT_PATH}/${channel}"
    ensure_directory_exists "${cur_path}"
    cd "${cur_path}" || exit
    run_download "${CHANNELS[$channel]}"
  done
}
